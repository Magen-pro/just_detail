...
async function init() {
  ...
  // If arriving from a "Record Payment →" link, open the modal pre-filled
  // for that specific booking/subscription instead of making the admin
  // pick it again from a long dropdown.
  const params = new URLSearchParams(window.location.search);
  const onetimeId = params.get('pay_onetime');
  const subscriptionId = params.get('pay_subscription');
  if (onetimeId) {
    await openPaymentModal({ type: 'one_time', id: onetimeId });
    window.history.replaceState({}, '', 'finances.html');
  } else if (subscriptionId) {
    await openPaymentModal({ type: 'subscription', id: subscriptionId });
    window.history.replaceState({}, '', 'finances.html');
  }
}
...
async function loadTodayMonthSummary() {
  ...
  const [todayPayments, todayExpenses, monthPayments, monthExpenses] = await Promise.all([
    supabaseClient.from('payments').select('amount').eq('payment_date', todayStr),
    supabaseClient.from('expenses').select('amount').eq('expense_date', todayStr),
    supabaseClient.from('payments').select('amount').gte('payment_date', monthStart).lte('payment_date', todayStr),
